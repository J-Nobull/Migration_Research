...
def avg_years(df, fips, col, y1, y2):
    """Calculate average of a column value for a FIPS across two years."""
    mask = df['FIPS'] == fips
    v1 = df[mask & (df['Year'] == y1)][col].iloc[0]
    v2 = df[mask & (df['Year'] == y2)][col].iloc[0]
    if pd.notna(v1) and pd.notna(v2):
        return int(round((v1 + v2) / 2))  # ← Round and convert to remain INT
    else:
        return np.nan

def fill(df, fips, col, year, value):
    """Fill a specific value for FIPS/Year/Column (only fills NaN)."""
    mask = (df['FIPS'] == fips) & (df['Year'] == year) & (df[col].isna())
...
def clean_census_data():
    ...
    # Ensure median columns are Int64 for imputation
    ...
    save_point(census, 'Census_prep.csv', f"{len(census):,} fixed N/As")
    return census
